...
const server = http.createServer(async (req, res) => {
  ...
  try {
    const cleanUrl = (req.url || '').replace(/\.php$/, '');

    if (cleanUrl === '/api/health') return send(res, 200, { ok: true });

    if (cleanUrl === '/api/state' && req.method === 'GET') {
      return send(res, 200, await readDb());
    }

    if (cleanUrl === '/api/state' && (req.method === 'PUT' || req.method === 'POST')) {
