...
export default function InvoiceDetailPage() {
  const { myInvoices, activeInvoiceId, navigate, updateInvoiceStatus, deleteInvoice } = useAppStore();
  ...
  if (!invoice) {
    return (
      <div className="max-w-3xl mx-auto card p-8 text-center anim-fade-up">
        <p className="text-gray-500 mb-4">Накладная не найдена</p>
        <button className="btn btn-primary" onClick={() => navigate('invoices')}>К списку</button>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-5 anim-fade-up">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <button className="btn btn-secondary" onClick={() => printInvoice(invoice)}><IconPrint size={16} />Печать</button>
          <button className="btn btn-primary" onClick={downloadPdf} disabled={pdfBusy}><IconDownload size={16} />{pdfBusy ? 'Готовлю...' : 'PDF'}</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card p-5 md:col-span-2">
          <p className="text-[12px] font-semibold text-gray-400 mb-2">Клиент</p>
        ...
        <div className="card p-5">
          <p className="text-[12px] font-semibold text-gray-400 mb-2">Курс</p>
      ...
      <div className="card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-[15px] font-bold text-gray-900">Статус накладной</p>
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="text-[16px] font-bold text-gray-900">Позиции</h2>
        </div>
        <div className="overflow-x-auto">
          <div className="min-w-[920px]">
            <div className="grid bg-slate-50 border-b border-slate-100 px-4 py-3 text-[12px] font-semibold text-gray-500" style={{ gridTemplateColumns: '48px 210px 120px 120px 120px 140px 150px' }}>
              <div>#</div><div>Товар</div><div>Кол-во</div><div>Цена USD</div><div>Цена RUB</div><div>Сумма USD</div><div>Сумма RUB</div>
            </div>
            {invoice.lines.map((line, idx) => (
              <div key={line.id} className="grid items-center px-4 py-4 border-b border-slate-100 last:border-b-0 text-[14px]" style={{ gridTemplateColumns: '48px 210px 120px 120px 120px 140px 150px' }}>
                ...
                <div className="pr-3">
                  ...
                  <p className="text-[12px] text-gray-400 truncate">Плотность: {line.productDensity} г/м²</p>
                </div>
            ...
            <div className="grid bg-slate-50 px-4 py-4 border-t border-slate-100" style={{ gridTemplateColumns: '48px 210px 120px 120px 120px 140px 150px' }}>
              <div className="col-start-6">
                <p className="text-[12px] text-gray-400 font-semibold">Итого USD</p>
                <p className="text-[20px] font-bold text-gray-900 text-money">${usd(invoice.totalUsd)}</p>
              </div>
              <div>
                <p className="text-[12px] text-gray-400 font-semibold">Итого RUB</p>
      ...
      <div className="flex justify-between pb-6">
        <button className="btn btn-danger" onClick={remove}><IconTrash size={16} />Удалить</button>
        <button className="btn btn-secondary" onClick={() => navigate('invoices')}>К списку</button>
      </div>
    </div>
  );
}